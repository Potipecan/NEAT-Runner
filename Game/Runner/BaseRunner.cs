...
public abstract class BaseRunner : KinematicBody2D
{
    ...
    public override void _PhysicsProcess(float delta)
    {
        ...
        //GD.Print($"{Velocity} {State} {Move} {Jump}");
        ...
        if (State == RunnerState.Grounded || State == RunnerState.Neutral) Velocity.y += Gravity * delta;
        ...
        for (int i = 0; i < collNum; i++)
        {
            ...
            else if (groups.Contains("Coins"))
            {
                //PickupCoin(collider);
            }
            else if (groups.Contains("Flags"))
            {
                //TouchFlag(collider);
            }
        ...
        else if (newVelocity.y > 0 && Velocity.x == newVelocity.x)
        {
            State = RunnerState.Neutral;
        }
